...
+function ($) {
    ...
    var BJUI = {
        ...
        alertMsg: {displayPosition: 'middlecenter', alertTimeout: 10000,displayMode:'fade',mmask:true}, //alertmsg display position && close timeout
        ajaxTimeout: 30000,
        ...
        initLayout: function () {
            ...
            if (ww < BJUI.ui.showSideWidth) {
               ...
               // $navtab.css('margin-left', (BJUI.ui.sidenavWidth + BJUI.ui.offsetWidth))
                $navtab.css('margin-left', (BJUI.ui.sidenavWidth+3))
            ...
            $sidenavbtn.off(eventName).on(eventName, function () {
               ...
               opts['margin-left'] = (BJUI.ui.sidenavWidth + 3)
                if (ww < BJUI.ui.showSideWidth)
                    opts['margin-left'] = 0
    ...
    $(function () {
        ...
        $.validator && $.validator.config({
            ...
            rules: {
                ...
                // not0:[/^(?!0$)/,'请输入大于0的正实数']
                // ,
                not0:[/([1-9]\d*(\.\d*[1-9])?)/,'请输入大于0的正实数']
                ,
                letters: [/^[a-z]+$/i, '{0}只能输入字母']
                ,
                customercode: [/^[A-Za-z0-9]+$/, '只能输入字母和数字']
                ,
                goodsnocode: [/^[A-Za-z0-9]+[-][0-9]+[-][0-9]+$/, '只能输入(字母/数字-数字-数字)的形式']
                ,
...
+function ($) {
    ...
    BJUI.setRegional('datagrid', {
        ...
        selectMoreMsg: 'Not selected more rows!',
    ...
    window.FRAG = {
        ...
        '        <a href="javascript:;" title="#first#">首页</a>' +
        '    </li>' +
        '    <li class="page-prev btn-nav">' +
        '        <a href="javascript:;" title="#prev#"><i class="fa fa-angle-double-left"></i></a>' +
        '    </li>' +
        '    #pageNumFrag#' +
        '    <li class="page-next btn-nav">' +
        '        <a href="javascript:;" title="#next#"><i class="fa fa-angle-double-right"></i></a>' +
        '    </li>' +
        '    <li class="page-last btn-nav">' +
        '        <a href="javascript:;" title="#last#">尾页</a>' +
        '    </li>' +
        '</ul>'
        ,
        ...
        + '    <li><button type="button" class="btn btn-default cancel" data-icon="remove">#cancel#</button></li>'
        + '    <li><button type="button" class="btn btn-red save" data-icon="save">#save#</button></li>'
        + '</ul>'
        ,
        ...
        '                <tr>' +
        '                    <td width="20"><a class="prev" href="javascript:;" title="#prev#"><i class="fa fa-angle-double-left"></i></a></td>' +
        '                    <td><select style="border-color:#e1e6eb;" name="year"></select></td>' +
        '                    <td><select style="border-color:#e1e6eb;" name="month"></select></td>' +
        '                    <td width="20"><a class="next" href="javascript:;" title="#next#"><i class="fa fa-angle-double-right"></i></a></td>' +
        '                </tr>' +
        '            </table>' +
        '        </div>' +
        '        <div class="body">' +
        ...
        '                </tr>' +
        '            </table>' +
        '            <button type="button" class="clearBtn btn btn-blue">#clear#</button>' +
...
+function ($) {
    ...
    Sidenav.prototype.ajaxHnav = function () {
        ...
        var createA = function (data) {
            ...
            liHtml.add('<a oncontextmenu = "customContextMenu(event)" ondragstart="return false" href="' + (data.url || 'javascript:;') + '" ')
            ...
            // liHtml.add('><i class="fa fa-'+ (data.icon || 'paper-plane') +'"></i>&nbsp;'+ data.name)
            liHtml.add('>' + data.name)
...
+function ($) {
    ...
    $(function () {
        var INIT_NAVTAB = function () {
            ...
            var mainTit, options = $.extend({}, Navtab.DEFAULTS, $main.data(), {
                id: 'main',
                title: mainTit
        ...
        if ($uploaders.length && WebUploader) {
            ...
            $uploaders.each(function (i, n) {
                ...
                if (waitCount && uploader) {
                    ...
                    if (!$form.find('.uploadBtn').hasClass('disabled')) {  //修改判断条件，不根据条数，根据上传按钮状态来判断
                        // 自动上传 
                        uploader.upload()
                        // 上传完成后触发
...
+function ($) {
    ...
    Alertmsg.prototype.confirm = function (msg, btnoptions) {
        ...
        var buttons = [
            {name: op.cancelName, call: op.cancelCall, cls: 'default', keyCode: BJUI.keyCode.ESC},
...
+function ($) {
    ...
    Datagrid.prototype.TOOLS = function () {
        ...
        var tools = {
            ...
            createTrsByData: function (data, refreshFlag) {
             ...
             //   that.$tfoot.trigger('resizeH.bjui.datagrid.tfoot')
            },
            // initTbody
                ...
                for (var i = 0, l = datas.length; i < l; i++) {
                    ...
                    for (var j = 0; j < modellength; j++) {
                        ...
                        if (n.calc) {
                         ...
                         // console.log("sum:");
                         // console.log(n.calc_sum);
                         Math.round(1000*(n.calc_sum))/1000.0
                         }
        ...
        if (options.showToolbar) {
            if (options.toolbarItem || options.toolbarCustom) {
                if (options.toolbarItem) {
                    ...
                    $.each(options.toolbarItem.split(','), function (i, n) {
                        ...
                        if (n === 'add') {
                            ...
                            that.$toolbar_edit = $(btnHtml).attr('data-icon', 'edit').addClass('btn-green').text(options.editName || BJUI.getRegional('datagrid.edit'))
                                ...
                                .on('click', function (e) {
                                    ...
                                    if (!$selectTrs.length) {
                                        ...
                                        $(this).alertmsg('warn', BJUI.getRegional('datagrid.selectMsg'),{displayPosition:'middlecenter',displayMode:'fade'})
                                    } else if ($selectTrs.length>1) {
                                        $(this).alertmsg('info', BJUI.getRegional('datagrid.selectMoreMsg'))
                                    }else {
                            ...
                            that.$toolbar_save = $(btnHtml).attr('data-icon', 'save').addClass('btn-green').text(options.saveName || BJUI.getRegional('datagrid.save'))
                                .appendTo($group)
                                .on('click', function (e) {
                                    that.doSaveEditRow()
                                })
                            ...
                            that.$toolbar_del = $(btnHtml).attr('data-icon', 'times').addClass('btn-red').text(options.delName || BJUI.getRegional('datagrid.del'))
                                ...
                                .on('click', function (e) {
                                    ...
                                    if ($selectTrs.length) {
                                        ...
                                        // $(this).alertmsg('warn', BJUI.getRegional('datagrid.selectMsg'))
                                        $(this).alertmsg('warn', BJUI.getRegional('datagrid.selectMsg'),{displayPosition:'middlecenter',displayMode:'fade'})
                                    }
                                })
                        } else if (n === 'refresh') {
                                    ...
                                    if (options.exportOption) {
                                        ...
                                        if (opts.options && opts.options.url) {
                                            if (!opts.options.data)
                                                opts.options.data = {}
                                            if (opts.options.form)
                                                opts.options.data = opts.options.form.serializeArray();
                ...
                if (options.toolbarCustom) {
                    var $custom, $custombox = $('<div style="display:inline-block;padding-left:5px;"></div>')
    ...
    Datagrid.prototype.initEvents = function ($trs) {
        ...
        $trs.on('click.bjui.datagrid.tr', function (e, checkbox) {
            ...
            .on('dblclick', function (e) {
                var $tr = $(this), options = that.options, index = $tr.index(), data, $selectedTrs = that.$tbody.find('> tr.' + that.classnames.tr_selected), $last = that.$lastSelect, checked, $lockTrs = that.$lockTbody && that.$lockTbody.find('> tr'), tools = that.tools, $editTd
                ...
                if (!options.editMode) return false;
                if(options.editMode.navtab)
                {
                    if(options.editMode.navtab.title=="新建杂费单")
                    {
                        return false;
                    }
                }
    ...
    Datagrid.prototype.initTfoot = function () {
        ...
        that.$boxF.append($('<div class="datagrid-wrap-f"></div>').append(that.$tableF))
        that.$boxF.insertAfter(that.$boxB)
        
        

        // that.$tfoot = $('<thead></thead>')
        that.$tfoot = $('<tfoot></tfoot>')

        $.each(columnModel, function (i, n) {
            if(n.name=="gridNumber")
            {
                var $th = $('<td class="datagrid-linenumber-td single-row" align="center" ><div class="datagrid-label">合计</div></td>')
            }
            else
            {
                var $th = $('<td><div></div></td>')
            }

            if (n.calc) {
                // var calc_html = '<div><div class="datagrid-calcbox">#tit#</div>#number#</div>'
                var calc_html = '<div align="center">#number#</div>'
                ...
                if (n.calc === 'avg')
                    ...
                    $th.html(calc_html.replace('#tit#', (n.calcTit || 'SUM')).replace('#number#', (Math.round(1000*(n.calc_sum))/1000.0)))
                else if (n.calc === 'max')
        ...
        that.$colgroupH = that.$colgroupF.clone()
        that.$colgroupB = that.$colgroupF.clone()
...
//resize
    Datagrid.prototype.colFoot = function () {
    ...
    Datagrid.prototype.initPaging = function () {
        ...
        $last.on('click', function () {
         ...
         $(document).on('afterLoad.bjui.datagrid', function () {
            // console.log("切换完成数据");
            that.colFoot();
         });
    ...
    Datagrid.prototype.add = function (addLocation, addData, noEdit) {
        ...
        if (options.isTree && that.$lastSelect) {
            ...
            var parentData = null; //添加的时候不需要add的数据
            if (parentData) {
    ...
    //没有确认框删除
    Datagrid.prototype.delAllRows = function (rows) {
        var that = this, options = that.options, keys = options.keys, beforeDelete = options.beforeDelete, confirmMsg = options.delConfirm, $trs
...
+function ($) {
    ...
    $(function () {
        var INIT_THEME = function () {
            ...
            if ($.cookie) {
                var themeName = $.cookie('bjui_theme') || 'green'
                ...
                if (options) {
                    ...
                    if (typeof options === 'object') {
                        ...
                        // 上传文件的单位(单位 + 类型)
                           ...
                           if(basePath !='')
                            downPath = basePath.replace('preview','download');
                        ...
                        var addFile = function (file, isuploaded, _index) {
                            if (!file && options.uploaded) {
                                $.each(options.uploaded.split(','), function (i, n) {
                                    var uploadedFile = {id: 'WU_FILE_UP_' + i, name: n.trim(), src: basePath + n.trim(), downsrc: downPath + n.trim()}
                            ...
                            var uploadedAttr = isuploaded && upunit === '张图片' ? ' style="cursor:pointer;" data-toggle="ajaxdownload" data-options="{url:\'' + file.downsrc + '\', mask:false}"' : '',
                                    ...
                                    case 0:
                                        if (isuploaded) {
                                            ...
                                            if (deletePath != '') {
                                                ...
                                                $.post(deletePath, {sysno: file.name}, function (response) {
                            ...
                            if (state === 'ready') {
                                ...
                                if (stats.uploadFailNum) {
                                    text = '已成功上传' + stats.successNum + upunit + '，' +
                                        stats.uploadFailNum + upunit + '上传失败，<a class="retry" href="#">重新上传</a> '
                                }
